...
((window, document, Math) => {
  ...
  function init() {
    ...
    addListener(colorArea, 'mousedown', _ => {
      addListener(document, 'mousemove', moveMarker);
    });

    addListener(colorArea, 'touchstart', _ => {
      document.addEventListener('touchmove', moveMarker, { passive: false });
    });

    addListener(colorMarker, 'mousedown', _ => {
      addListener(document, 'mousemove', moveMarker);
    });

    addListener(colorMarker, 'touchstart', _ => {
      document.addEventListener('touchmove', moveMarker, { passive: false });
    });

    addListener(colorValue, 'change', _ => {
      if (currentEl || settings.inline) {
    ...
    addListener(clearButton, 'click', _ => {
      pickColor('');
      closePicker();
    });

    addListener(closeButton, 'click', _ => {
      pickColor();
      closePicker();
    });
    ...
    addListener(document, 'mouseup', _ => {
      document.removeEventListener('mousemove', moveMarker);
    });

    addListener(document, 'touchend', _ => {
      document.removeEventListener('touchmove', moveMarker);
    });

    addListener(document, 'mousedown', _ => {
      picker.classList.remove('clr-keyboard-nav');
      closePicker();
    });
